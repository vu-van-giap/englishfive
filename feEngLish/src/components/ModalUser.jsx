...
const ModalUser = ({ isOpen, onClose, user, mode, onSuccess }) => {
    ...
    const validate = () => {
        ...
        if (!formData.fullname.trim()) newErrors.fullname = "Full name không được để trống";
        if (mode === "add") {
            if (!formData.role) newErrors.role = "Vui lòng chọn role";
            if (!formData.password) newErrors.password = "Password không được để trống";
            else if (formData.password.length < 6) newErrors.password = "Password phải ít nhất 6 ký tự";
        }
        setErrors(newErrors);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (mode === "add") {
                ...
                await updateUser(user._id, { username: formData.username, fullname: formData.fullname });
