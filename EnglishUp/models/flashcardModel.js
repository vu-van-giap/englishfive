...
async function flashcardModel(fastify) {
  ...
  const normalizeInput = (data = {}) => {
    ...
    const topic =
      data.topic === null || data.topic === undefined
        ? null
  ...
  return {
     ...
     */
    async getAllFlashcards({ page = 1, limit = 0 } = {}) {
      try {
        // Nếu limit = 0, trả về tất cả không phân trang
        if (limit === 0) {
          const data = await collection.find({}).toArray();
          const total = data.length;
          return { data, total, page: 1, limit: 0 };
        }
     ...
     * @returns {Promise<{data: Array, total: number, page: number, limit: number}>}
     */
    async getFlashcardsByTopic(topic, { page = 1, limit = 0 } = {}) {
      try {
        // Nếu limit = 0, trả về tất cả không phân trang
        if (limit === 0) {
     ...
     * @returns {Promise<{data: Array, total: number, page: number, limit: number}>}
     */
    async searchFlashcards(query, { page = 1, limit = 0 } = {}) {
      try {
        // Nếu limit = 0, trả về tất cả không phân trang
        if (limit === 0) {
          const searchRegex = new RegExp(query, "i");
          const searchQuery = {
            $or: [{ front: searchRegex }, { back: searchRegex }],
          };
